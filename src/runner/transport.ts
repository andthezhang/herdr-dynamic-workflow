 ...
 * writes HERDR_FLOW_OUT on ITS host, so the harvest read — and the schema
 ...
 *   host's resolved absolute binary path (SPEC D13).
 */
...
export interface HerdrTransport {
  ...
  /** Write a text file on the transport's host, creating parent directories. */
  writeTextFile(filePath: string, content: string): Promise<void>;
  /** Ensure a directory exists on the transport's host (mkdir -p). */
...
/** This computer: Herdr over its Unix socket, files over node:fs. */
