/**
 * Engine-core runtime tests, ported from pi-dynamic-workflows v3.5.1 (MIT)
 * tests/workflow-runtime.test.ts. Pi-SDK-dependent cases dropped; imports
 * adapted to the vendored engine.
 */
...
test("runWorkflow plumbs opts.tier through to the agent with correct precedence", async () => {
  // Regression guard: tier must reach the runner. Precedence: explicit model >
  // tier > phase model.
  ...
  //    inside the runner) wins over the phase model; tier is forwarded.
...
test("sequential nested workflow() calls at the same depth get distinct child run ids (no cross-child id/deltaKey collision)", async () => {
  ...
  // id), corrupting SharedStore deltas and misattributing events. child1's
  // agent() call is deliberately left un-awaited — realistically, that's
  // exactly when the collision bites: the stray can still be in
  // SharedRuntime.inFlight (only the top-level frame drains, not each nested
  // frame) when child2 starts and mints an id.
...
test("runWorkflow initialTokenUsage seeds the run-wide budget so it holds cumulatively across resume", async () => {
  // Simulates what a manager's resume() passes: a prior execution already
...
test("runWorkflow initialTokenUsage integrates correctly with phase() sub-budgets (seeded baseline isn't corrupted)", async () => {
  ...
  // phase() call, so a seed doesn't make the phase's OWN ceiling trip any
  // sooner than usual — it only shifts the visible baseline.
...
test("a fan-out past maxAgents cancels queued agents instead of draining the reserved queue", async () => {
  ...
  // limiter. The breach short-circuits the still-queued agents so at most
  // ~concurrency of them execute.
  const fanout = 100;
  const maxAgents = 50;
  const concurrency = 4;
  const calls = { count: 0 };
...
return xs`;
  ...
  // The run drains every in-flight agent() call (including these gate-blocked
  // ones) before its own promise settles — so `run` will NOT reject until
  // `gate` resolves. Release it concurrently instead of after awaiting the
...
test("sibling parallel() batches are isolated: one breaching maxAgents does not cancel the other", async () => {
  ...
  // unrelated fan-out) breached the shared cap.
  const maxAgents = 10;
  const concurrency = 2;
  const runner = {
    async run(prompt: string) {
...
return results`;
  ...
  // Completion order is deliberately REVERSED (task 3 finishes first) so this
  // actually asserts input-order preservation, not just completion order.
  const agent = {
    async run(prompt: string) {
      const n = Number(prompt.slice(-1));
      await new Promise((resolve) => setTimeout(resolve, (3 - n) * 20));
      return `ok:${prompt}`;
    },
  };

  const result = await runWorkflow<unknown[]>(script, { agent, persistLogs: false, concurrency: 3 });
  assert.deepEqual(
    result.result,
    ["ok:task 1", "ok:task 2", "ok:task 3"],
    "parallel() results must line up with input order even when completion order differs",
  );
});
...
return results`;
  ...
  assert.deepEqual(
    result.result,
    ["done(stage2 done(stage1 a))", "done(stage2 done(stage1 b))"],
    "each item's value must flow stage1 -> stage2, results in item order",
  );
  // Per-item stage sequencing: every item's stage2 prompt embeds its own
  // stage1 result and comes after that stage1 prompt.
  for (const item of ["a", "b"]) {
    const s1 = log.indexOf(`stage1 ${item}`);
    const s2 = log.indexOf(`stage2 done(stage1 ${item})`);
    assert.ok(s1 >= 0, `stage1 must run for item ${item}`);
    assert.ok(s2 > s1, `stage2 must run after stage1 for item ${item}`);
  }
  assert.equal(log.length, 4, "exactly two stages per item");
});

test("pipeline is barrier-free between stages (SPEC D5): item A reaches stage 2 while B is still in stage 1", async () => {
  const script = `export const meta = { name: 'pipeline_no_barrier', description: 'no stage barrier' }
const results = await pipeline(['a','b'], item => agent('s1 ' + item), prev => agent('s2 ' + prev))
return results`;

  // Gate: item b's stage 1 completes only once item a's stage 2 has STARTED.
  // With the (wrong) per-stage barrier — awaiting all of stage 1 before any
  // stage 2 — a's stage 2 could not start until b's stage 1 finished; the
  // fallback timer would then release b first and the flag stays false.
  let aStage2Started = false;
  let aStage2StartedBeforeBStage1Finished = false;
  let releaseB!: () => void;
  ...
  const agent = {
    async run(prompt: string) {
      if (prompt === "s2 done-s1 a") {
        aStage2Started = true;
        releaseB();
      }
      if (prompt === "s1 b") {
        // Fallback timer keeps a regression from deadlocking the suite; the
        // flag below is what actually fails it.
        await Promise.race([bGate, new Promise((resolve) => setTimeout(resolve, 2000))]);
        aStage2StartedBeforeBStage1Finished = aStage2Started;
      }
      return `done-${prompt}`;
    },
  };

  const result = await runWorkflow<string[]>(script, { agent, persistLogs: false, concurrency: 4 });
  assert.ok(
    aStage2StartedBeforeBStage1Finished,
    "stage 2 of a fast item must start while a slow item is still in stage 1 — no per-stage barrier",
  );
  assert.deepEqual(result.result, ["done-s2 done-s1 a", "done-s2 done-s1 b"]);
});
...
return 1`;
  ...
  assert.ok(Array.isArray(result.logs), "result.logs should be an array");
  assert.deepEqual(result.logs, [], "nothing logged -> the logs array must actually be empty");
});

// ─── Runtime determinism hardening ──────────────────────────────────────────────
...
test("a parent script that catches a nested workflow()'s uncaught child error can still run agents afterward (isTopLevelRun gate)", async () => {
  // Only the TOP-level frame is allowed to seal shared.runFatalController — a
  // NESTED frame reaching its own catch must never seal it, because the error
  // hasn't finished propagating yet: the parent script may still catch
  // workflow()'s rejection and continue normally.
  const { state, runner } = abortAwareAgent(20);
  const child = `export const meta = { name: 'child', description: 'c' }
