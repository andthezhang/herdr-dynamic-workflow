...
test("parseWorkflowInvoke validates Claude-compatible wf_ run ids", () => {
  const ok = parseWorkflowInvoke({ resumeFromRunId: "wf_k2ab91" });
  assert.equal(ok.resumeFromRunId, "wf_k2ab91");
  assert.throws(() => parseWorkflowInvoke({ resumeFromRunId: "run-mt5cpbpy-i2ab" }), /resumeFromRunId must match/);
  assert.throws(() => parseWorkflowInvoke({ resumeFromRunId: "wf_short" }), /resumeFromRunId must match/);
