/**
 * SharedStore tests, ported from pi-dynamic-workflows v3.5.1 (MIT)
 * tests/shared-store.test.ts. The pi `store_put`/`store_get` MCP tools are not
 * ported (see the TODO seam in src/engine/shared-store.ts); the integration
 * cases are adapted to the engine's AgentStoreHandle seam
 * (AgentRunOptions.store), which preserves the same delta-tracking semantics.
 */
import assert from "node:assert/strict";
import test from "node:test";
import type { AgentStoreHandle } from "../src/engine/shared-store.js";
import { SharedStore } from "../src/engine/shared-store.js";
...
test("each runWorkflow call gets an isolated SharedStore: run 2 does not see run 1's writes", async () => {
  ...
  const agent = {
    async run(prompt: string, opts: { store?: AgentStoreHandle }) {
      if (prompt === "put") {
        opts.store?.put("shared_key", "run1");
        return "wrote";
      }
      // prompt === "get"
      const res = opts.store?.get("shared_key");
      readsByRun[prompt] = res?.found ?? false;
      return "read";
    },
  };
...
test("the store handle reaches every agent regardless of agentType (the allowlist-proof seam)", async () => {
  // In pi, store_put/store_get were injected AFTER the agentType tools
  // allowlist so a restrictive definition couldn't strip them. The vendored
  // engine's equivalent guarantee: AgentRunOptions.store is always present,
  // agentType or not, and the definition's prompt/kind still bind.
  let observedStore: AgentStoreHandle | undefined;
  let observedInstructions: string | undefined;
  let observedKind: string | undefined;

  const agent = {
    async run(_prompt: string, opts: { store?: AgentStoreHandle; instructions?: string; kind?: string }) {
      observedStore = opts.store;
      observedInstructions = opts.instructions;
  ...
  const agentRegistry = {
    resolve: (name: string) =>
      name === "read-only-auditor" ? { prompt: "You audit code read-only.", kind: "claude" } : undefined,
  };

  const script = `
    export const meta = { name: "store-seam-test", description: "store seam test" };
    return await agent("audit", { agentType: "read-only-auditor" });
  `;

  await runWorkflow(script, { agent, cwd: process.cwd(), agentRegistry });

  assert.ok(observedStore, "the store handle must be injected for agentType-bound agents too");
  assert.equal(typeof observedStore?.put, "function");
  assert.equal(typeof observedStore?.get, "function");
  assert.match(observedInstructions ?? "", /audit code read-only/, "the definition's role prompt reaches the runner");
  assert.equal(observedKind, "claude", "the definition's kind reaches the runner");
});

// ─── Nested workflow() delta-collision regression ────────────────────────────
...
test("nested workflow() concurrent with its parent does not collide on shared-store deltas", async () => {
  ...
  const journal: import("../src/engine/workflow.js").JournalEntry[] = [];

  const agent = {
    async run(prompt: string, opts: { store?: AgentStoreHandle }) {
      if (prompt.startsWith("put:")) {
        const [, key, val] = prompt.split(":");
        opts.store?.put(key as string, val);
        return `wrote ${key}`;
      }
      if (prompt.startsWith("get:")) {
        const [, key] = prompt.split(":");
        const res = opts.store?.get(key as string);
        return { key, found: res?.found, value: res?.value };
      }
      return "ok";
    },
  };
...
test("resume after nested workflow(): parent and child journal entries cache-hit independently despite both indexing at 0", async () => {
  ...
  // the parent's own callIndex-0 entry. Before namespacing, a per-index
  // journal map meant whichever entry was recorded LAST clobbered the other,
  // so a resume could never cache-hit both frames.
  const journal: import("../src/engine/workflow.js").JournalEntry[] = [];
...
test("resume degrades gracefully (never corrupts) when replaying a pre-namespacing legacy journal", async () => {
  ...
  // `runId` field on disk. A manager's resume() falls back to the run's own
  // top-level runId for such entries, which this test replicates directly
  // against runWorkflow.
  const journal: import("../src/engine/workflow.js").JournalEntry[] = [];
  const firstCalls = { count: 0 };
  const first = await runWorkflow<{ inner: string; outer: string }>(nestedResumeScript, {
  ...
  const legacyEntries = journal.map((e) => {
    ...
    return rest as import("../src/engine/workflow.js").JournalEntry;
  });
...
test("resume: an upstream parent-call miss cuts the nested child off from the journal (child re-runs live)", async () => {
  // Propagating resumeJournal into a nested workflow() call is only safe
  // while the PARENT's own longest-unchanged-prefix is still intact.
  ...
  const agent = {
    async run(prompt: string, opts: { store?: AgentStoreHandle }) {
      if (prompt === "call") {
        callAttempts++;
        if (callAttempts === 1) {
          opts.store?.put("poisonedOnly", "should-never-survive");
          throw new Error("transient failure");
        }
        opts.store?.put("shared", "good");
        return "call-done";
      }
      ...
      return opts.store?.get("poisonedOnly")?.found;
    },
  };
  const journal: import("../src/engine/workflow.js").JournalEntry[] = [];
  ...
  const agent = {
    async run(prompt: string, opts: { store?: AgentStoreHandle }) {
      ...
      if (prompt.startsWith("get:")) {
        const [, key] = prompt.split(":");
        const res = opts.store?.get(key as string);
        writeCalls[key as string] = String(res?.value ?? "MISSING");
        return `got ${key}:${writeCalls[key as string]}`;
      }
      return "ok";
    },
  };
