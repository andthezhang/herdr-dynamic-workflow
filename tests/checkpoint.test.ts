/**
 * checkpoint() tests, ported from pi-dynamic-workflows v3.5.1 (MIT)
 * tests/checkpoint.test.ts with imports adapted to the vendored engine.
 */
import assert from "node:assert/strict";
import test from "node:test";
import type { JournalEntry } from "../src/engine/workflow.js";
...
return { r }`;
  ...
  // Edited script: same prompt/kind/choices, only `default` changed. `default`
  // is part of the checkpoint hash, so this must cache-miss and take the NEW
  // default live instead of replaying the STALE journaled "A" reply.
...
return { r }`;
  ...
  // `headless` is part of the hash, so this must cache-miss and exercise the
  // new abort behavior live instead of silently replaying the old reply.
