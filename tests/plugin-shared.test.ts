...
test("formatWorkspaceLabel leads with the caller name and suffixes last-4 of runId", () => {
  assert.equal(formatWorkspaceLabel("PR 412 reviews", "wf_mt5cpbpy-i2ab"), "PR 412 reviews · i2ab");
  assert.equal(formatWorkspaceLabel("  hello\nworld  ", "wf_xxxx-ab12"), "hello world · ab12");
  assert.equal(formatWorkspaceLabel("   ", "wf_xxxx-ab12"), "workflow · ab12");
});
...
test("newRunId mints unique, journal-filename-safe ids", () => {
  ...
  for (const id of ids) assert.match(id, /^wf_[a-z0-9-]{6,}$/);
});
