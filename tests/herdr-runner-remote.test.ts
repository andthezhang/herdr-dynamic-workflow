...
async function startHarness(runnerOverrides: Record<string, unknown> = {}): Promise<Harness> {
  ...
  const runner = new HerdrAgentRunner({
    ...
    stateDir,
    defaults: { kind: "claude", cwd: "/workspace/herdr-dynamic-workflow" },
    session: "flow",
...
test("ssh:'example-host' runs the whole D11 topology over ssh and harvests the remote output file", async () => {
  const harness = await startHarness();
  const mac = harness.host("example-host");
...
test("any ssh host name is a destination; herdr is found with a login-shell probe and the local socket is untouched", async () => {
  const harness = await startHarness();
  const box = harness.host("example-host");
  box.onPrompt = remoteWritesResult("found herdr");
  try {
    assert.equal(await harness.runner.run("go", { label: "w", ssh: "example-host" }), "found herdr");
    assert.ok(
...
test("omitting ssh keeps the call on the local socket; the two destinations get their own transports", async () => {
  ...
  try {
    ...
    assert.equal(await harness.runner.run("there", { label: "b", ssh: "example-host" }), "ran remote");
...
test("two calls on the SAME ssh host share one transport and one workspace", async () => {
  ...
  try {
    assert.equal(await harness.runner.run("one", { label: "a", ssh: "example-host" }), "ok");
    assert.equal(await harness.runner.run("two", { label: "b", ssh: "example-host" }), "ok");
...
test("remote harvest fallback: no output file and no schema falls back to agent read's raw pane text", async () => {
  ...
  try {
    const result = await harness.runner.run("talk to the screen", { label: "w", ssh: "example-host" });
...
test("the kind's permission flags travel over ssh too", async () => {
  ...
  mac.onPrompt = remoteWritesResult("ok");
  try {
    await harness.runner.run("go", { label: "w", ssh: "example-host", kind: "codex", model: "gpt-5" });
...
test("worktree isolation (call-site cwd) combined with ssh is an explicit validation error, never a silent degrade", async () => {
  ...
  try {
    await assert.rejects(
      harness.runner.run("isolated", { label: "w", ssh: "example-host", cwd: "/tmp/worktrees/wt-1" }),
      (error: unknown) => {
        assert.ok(error instanceof WorkflowError);
        assert.equal(error.code, WorkflowErrorCode.SCRIPT_VALIDATION_ERROR);
        assert.equal(error.recoverable, false);
    ...
    assert.equal(harness.host("example-host").execs.length, 0, "rejected before any ssh traffic");
  ...
  try {
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        assert.equal(details.ssh, "example-host");
        assert.equal(
          details.attachCommand,
          `ssh -t example-host 'bash -lc '\\''herdr --session flow agent attach flow-resc-0'\\'''`,
    ...
    assert.ok(record);
    assert.equal(record.ssh, "example-host");
...
test("a blocked ssh call under the default fail policy tears the remote tab down", async () => {
  ...
  try {
    await assert.rejects(
      harness.runner.run("one", { label: "a", ssh: "example-host" }),
    ...
    assert.equal(await harness.runner.run("two", { label: "b", ssh: "example-host" }), "second ok");
  } finally {
    await harness.close();
  }
});
